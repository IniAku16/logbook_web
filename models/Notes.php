...
class NoteModel {
    ...
    public function create($date, $desc, $id_area, $jenis, $target, $material, $user_id) {
        $sql = "INSERT INTO notes (date, description, id_area, jenis, target, material, user_id) VALUES (?,?,?,?,?,?,?)";
        $stmt = $this->db->prepare($sql);
        $stmt->bind_param("ssisssi", $date, $desc, $id_area, $jenis, $target, $material, $user_id);
        return $stmt->execute();
    }

     public function update($id, $date, $desc, $id_area, $jenis, $target, $material, $user_id) {
